...
export default function EnterpriseDashboard({ onNavigate, isDarkMode, toggleDarkMode }: { onNavigate: (view: ViewState) => void, isDarkMode: boolean, toggleDarkMode: () => void }) {
  ...
  if (!enterprise) {
    ...
    return (
      <div className="min-h-screen bg-[#f3f3f4] dark:bg-[#0a0a0a] flex items-center justify-center p-4">
        <div className="bg-white dark:bg-[#111] max-w-xl w-full p-8 rounded-sm shadow-xl border border-[#cfc4c5] dark:border-[#333] text-center">
          ...
          <p className="text-[#4c4546] dark:text-[#a0a0a0] mb-8 leading-relaxed font-sans text-[15px]">
          ...
          <div className="bg-[#f9f9f9] dark:bg-[#1a1a1a] border border-[#cfc4c5] dark:border-[#333] rounded-sm p-4 text-left mb-8 space-y-3">
  ...
  // Dashboard state
  return (
    <div className="min-h-screen bg-[#f9f9f9] dark:bg-[#0a0a0a] text-black dark:text-white font-sans selection:bg-black selection:text-white">
      {/* Header */}
      <header className="sticky top-0 z-40 bg-white/80 dark:bg-black/80 backdrop-blur-md border-b border-[#cfc4c5] dark:border-[#333]">
        <div className="max-w-6xl mx-auto px-4 lg:px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Building2 className="w-5 h-5" />
            <h1 className="font-sans font-bold tracking-tight text-[16px]">{enterprise.name} <span className="font-mono font-normal text-[#7e7576] text-xs ml-2 tracking-widest uppercase">Enterprise</span></h1>
          </div>
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white dark:bg-[#111] border border-[#cfc4c5] dark:border-[#333] p-6 rounded-sm">
                  <div className="text-[#7e7576] font-mono text-[11px] font-bold uppercase tracking-widest mb-4">Total Seats</div>
                ...
                <div className="bg-white dark:bg-[#111] border border-[#cfc4c5] dark:border-[#333] p-6 rounded-sm">
                  <div className="text-[#7e7576] font-mono text-[11px] font-bold uppercase tracking-widest mb-4">Avg CTR</div>
                  <div className="text-5xl font-sans font-bold">12.4%</div>
                </div>
              </div>

              <div className="bg-white dark:bg-[#111] border border-[#cfc4c5] dark:border-[#333] rounded-sm p-6">
                <h3 className="font-sans font-bold text-lg mb-6">Recent Employee Growth</h3>
                <div className="h-48 flex items-end gap-2 text-[#7e7576] font-mono text-[10px]">
                  {/* Mock Chart */}
                  {[4, 12, 8, 15, 20, 18, 25].map((val, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center gap-2">
                       <div className="w-full bg-black/10 dark:bg-white/10 rounded-t-sm transition-all hover:bg-black dark:hover:bg-white" style={{ height: `${(val/25)*100}%` }}></div>
                       <span>Day {i+1}</span>
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex justify-between items-end">
                 <div>
                   ...
                   <p className="text-[#7e7576] font-mono text-[12px]">Manage user accounts & bios centrally.</p>
                 </div>
                 <button 
                   onClick={() => setIsCreatingEmployee(true)}
                   className="bg-black text-white dark:bg-white dark:text-black px-4 py-2 rounded-sm font-mono text-[12px] font-bold uppercase px-6 flex items-center gap-2 hover:opacity-80 transition-opacity"
                 >
              ...
              {(isCreatingEmployee || editingEmployee) && (
                <div className="bg-white dark:bg-[#111] border border-[#cfc4c5] dark:border-[#333] rounded-sm p-6 mb-6">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <button onClick={() => { setIsCreatingEmployee(false); setEditingEmployee(null); setEmployeeForm({ email: '', password: '', full_name: '', headline: '', bio: '', username: '' }); }} className="text-[#7e7576] hover:text-black dark:hover:text-white"><X className="w-5 h-5"/></button>
                  </div>
                    ...
                    {!editingEmployee && (
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <input required type="email" value={employeeForm.email} onChange={e => setEmployeeForm({...employeeForm, email: e.target.value})} className="w-full px-3 py-2 border border-[#cfc4c5] dark:border-[#333] rounded-sm bg-transparent outline-none focus:border-black dark:focus:border-white text-[14px]" />
                        </div>
                        <div>
                          <label className="block font-mono text-[11px] font-bold text-[#4c4546] dark:text-[#a0a0a0] uppercase mb-1">Temporary Password</label>
                          <input required type="password" value={employeeForm.password} onChange={e => setEmployeeForm({...employeeForm, password: e.target.value})} className="w-full px-3 py-2 border border-[#cfc4c5] dark:border-[#333] rounded-sm bg-transparent outline-none focus:border-black dark:focus:border-white text-[14px]" />
                        </div>
                      </div>
                    )}
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input required type="text" value={employeeForm.full_name} onChange={e => setEmployeeForm({...employeeForm, full_name: e.target.value})} className="w-full px-3 py-2 border border-[#cfc4c5] dark:border-[#333] rounded-sm bg-transparent outline-none focus:border-black dark:focus:border-white text-[14px]" />
                      </div>
                      <div>
                        <label className="block font-mono text-[11px] font-bold text-[#4c4546] dark:text-[#a0a0a0] uppercase mb-1">Username (URL slug)</label>
                        <input type="text" value={employeeForm.username} onChange={e => setEmployeeForm({...employeeForm, username: e.target.value})} className="w-full px-3 py-2 border border-[#cfc4c5] dark:border-[#333] rounded-sm bg-transparent outline-none focus:border-black dark:focus:border-white text-[14px]" />
                      </div>
                    </div>
                    <div>
                      <label className="block font-mono text-[11px] font-bold text-[#4c4546] dark:text-[#a0a0a0] uppercase mb-1">Headline</label>
                      <input type="text" value={employeeForm.headline} onChange={e => setEmployeeForm({...employeeForm, headline: e.target.value})} className="w-full px-3 py-2 border border-[#cfc4c5] dark:border-[#333] rounded-sm bg-transparent outline-none focus:border-black dark:focus:border-white text-[14px]" />
                    </div>
                    <div>
                      <label className="block font-mono text-[11px] font-bold text-[#4c4546] dark:text-[#a0a0a0] uppercase mb-1">Bio</label>
                      <textarea rows={3} value={employeeForm.bio} onChange={e => setEmployeeForm({...employeeForm, bio: e.target.value})} className="w-full px-3 py-2 border border-[#cfc4c5] dark:border-[#333] rounded-sm bg-transparent outline-none focus:border-black dark:focus:border-white text-[14px]" />
                    </div>
                    <button type="submit" className="bg-black text-white px-6 py-2 rounded-sm font-mono text-[12px] font-bold uppercase w-full">
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-[#cfc4c5] dark:border-[#333] bg-[#f9f9f9] dark:bg-[#1a1a1a]">
                      <th className="p-4 font-mono text-[11px] font-bold uppercase text-[#7e7576]">Name</th>
                      <th className="p-4 font-mono text-[11px] font-bold uppercase text-[#7e7576]">Role</th>
                      <th className="p-4 font-mono text-[11px] font-bold uppercase text-[#7e7576]">Link</th>
                      <th className="p-4 font-mono text-[11px] font-bold uppercase text-[#7e7576]">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {employees.map(emp => (
                      <tr key={emp.id} className="border-b border-[#cfc4c5] dark:border-[#333] last:border-0 hover:bg-[#f3f3f4] dark:hover:bg-[#1a1a1a]">
                        <td className="p-4 font-sans text-[14px] font-medium">{emp.full_name}</td>
                        <td className="p-4 font-sans text-[14px] text-[#7e7576]">{emp.headline || '-'}</td>
                        ...
                        <td className="p-4 flex items-center gap-3">
                          <button 
                            ...
                            className="text-[#7e7576] hover:text-black dark:hover:text-white transition-colors"
                          >
                            Edit
                          </button>
                          <button onClick={() => handleDeleteEmployee(emp.id)} className="text-[#7e7576] hover:text-red-500 transition-colors">
                    ...
                    {employees.length === 0 && (
                      <tr>
                        <td colSpan={4} className="p-8 text-center text-[#7e7576] font-mono text-sm">No employees added yet.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
          ...
          {activeTab === 'branding' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-2xl">
              <div>
                 ...
                 <p className="text-[#7e7576] font-mono text-[12px]">Settings here will reflect globally across all employee profiles.</p>
              </div>

              <form onSubmit={handleSaveBranding} className="bg-white dark:bg-[#111] border border-[#cfc4c5] dark:border-[#333] rounded-sm p-6 space-y-6">
                <div>
                  <label className="block font-mono text-[11px] font-bold text-[#4c4546] dark:text-[#a0a0a0] uppercase mb-1">Company Name</label>
                  <input required type="text" value={brandForm.name} onChange={e => setBrandForm({...brandForm, name: e.target.value})} className="w-full px-3 py-2 border border-[#cfc4c5] dark:border-[#333] rounded-sm bg-transparent outline-none focus:border-black dark:focus:border-white text-[14px]" />
                </div>
                
                <div>
                  <label className="block font-mono text-[11px] font-bold text-[#4c4546] dark:text-[#a0a0a0] uppercase mb-1">Company Logo URL</label>
                  <input type="text" placeholder="https://..." value={brandForm.logo_url} onChange={e => setBrandForm({...brandForm, logo_url: e.target.value})} className="w-full px-3 py-2 border border-[#cfc4c5] dark:border-[#333] rounded-sm bg-transparent outline-none focus:border-black dark:focus:border-white font-mono text-[12px]" />
                </div>

                <div className="grid grid-cols-2 gap-6">
                  <div>
                    <label className="block font-mono text-[11px] font-bold text-[#4c4546] dark:text-[#a0a0a0] uppercase mb-2">Brand Color</label>
                    <div className="flex gap-3 items-center">
                       <input type="color" value={brandForm.brand_color} onChange={e => setBrandForm({...brandForm, brand_color: e.target.value})} className="w-10 h-10 border-0 p-0 rounded-sm cursor-pointer" />
                       <span className="font-mono text-sm uppercase">{brandForm.brand_color}</span>
                    </div>
                  </div>
                  <div>
                    <label className="block font-mono text-[11px] font-bold text-[#4c4546] dark:text-[#a0a0a0] uppercase mb-2">Global Typography</label>
                    <select 
                      value={brandForm.brand_font} 
                      onChange={e => setBrandForm({...brandForm, brand_font: e.target.value})}
                      className="w-full px-3 py-2 border border-[#cfc4c5] dark:border-[#333] rounded-sm bg-transparent outline-none focus:border-black dark:focus:border-white font-sans text-[14px]"
                    >
                ...
                <button type="submit" className="bg-black text-white px-6 py-3 rounded-sm font-mono text-[12px] font-bold uppercase w-full hover:bg-opacity-90">
          ...
          {activeTab === 'billing' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-xl">
              <div>
                 ...
                 <p className="text-[#7e7576] font-mono text-[12px]">Manage your enterprise subscription.</p>
              </div>

              <div className="bg-white dark:bg-[#111] border border-[#cfc4c5] dark:border-[#333] rounded-sm p-6 space-y-6">
                 ...
                 <div>
                   ...
                   <div className="h-2 w-full bg-[#f3f3f4] dark:bg-[#222] rounded-full overflow-hidden">
