...
content = content.replace(
  /<img src=\{showQR \? \`https:\/\/api\.qrserver\.com\/v1\/create-qr-code\/\?size=150x150&data=https:\/\/chipng\.com\/\$\{profile\.username \|\| ''\}\` : coverUrl\} alt="Cover\/QR" className=\{\`w-full h-full \$\{showQR \? 'object-contain bg-white dark:bg-\\[#1a1c1c\\] p-2' : 'object-cover'\}\`\} \/>/g,
  `{showQR ? (
