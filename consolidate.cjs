...
// We will inject the new desktop tab bar
const oldTabs = /<div className="mb-8 overflow-x-auto whitespace-nowrap scrollbar-hide \[\&::-webkit-scrollbar\]:hidden" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>[\s\S]*?<\/button>\s*<\/div>\s*<\/div>/;

const newDesktopTabs = `<div className="mb-8 overflow-x-auto whitespace-nowrap scrollbar-hide [&::-webkit-scrollbar]:hidden hidden md:block" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
...
code = code.replace(oldTabs, newDesktopTabs);

// For the activeTab conditional rendering, let's map them simply:
// shop -> ebooks
code = code.replace(
...
// We have activeTab === 'gallery', 'nfc', 'buy-nfc', 'appearance' which we need to group under 'settings'
// The easiest way is to change all their conditionals to `activeTab === 'settings'` and let them all render sequentially when in settings.
// Let's do that:
code = code.replace(
...
// Remove the `) : profile && activeTab === 'gallery' ? (` block wrapper
code = code.replace(
  /\s*\) : profile && activeTab === 'gallery' \? \(\s*<div className="flex flex-col gap-8 max-w-4xl mx-auto w-full">/,
  ''
);
// Same for nfc
code = code.replace(
  /\s*\) : profile && activeTab === 'nfc' \? \(/,
  ''
);
// Same for buy-nfc
code = code.replace(
...
// Clean up closing divs for gallery and buy-nfc that we removed the opening tags for
code = code.replace(
  /<\/div>\s*<\/div>\s*\) : profile && activeTab === 'appearance' \? \(/, // Wait, I already replaced appearance...
  `</div></div>` // I'll do this carefully via manual string replace below to ensure I don't break JSX.
);

fs.writeFileSync('consolidate.cjs.tmp', code);
